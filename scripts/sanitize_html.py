...
def sanitize_html(input_file: str, output_file: str) -> bool:
    ...
    try:
        ...
        sanitized_content = re.sub(
            r"(Account\s*ID:?\s*)[A-Za-z0-9_-]+", r"\1REDACTED", html_content
        )
        sanitized_content = re.sub(
            r"(Account\s*Key:?\s*)[A-Za-z0-9_-]+", r"\1REDACTED", sanitized_content
        )
        sanitized_content = re.sub(
            r"(Client\s*ID:?\s*)[A-Za-z0-9_-]+", r"\1REDACTED", sanitized_content
        )
        sanitized_content = re.sub(
            r"(User\s*ID:?\s*)[A-Za-z0-9_-]+", r"\1REDACTED", sanitized_content
        )
